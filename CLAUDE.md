...
```
src/styles.css   → 디자인 토큰 정의 (@theme inline, :root, .dark)
  ↓ tailwindcss CLI
...
- 시맨틱 토큰은 `src/styles.css`에 CSS 커스텀 프로퍼티로 정의 — `:root` (라이트) + `.dark` (다크)
...
색상은 시맨틱 네이밍 (accent = emerald). `src/styles.css` `:root` / `.dark`에 정의:
...
- `<html class="dark">` 동기화 스크립트로 다크모드 토큰 적용
