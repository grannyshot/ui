...
### Tailwind dark: 프리픽스 연동

grannyshot-ui 컴포넌트는 `--gs-*` CSS 변수로 자동 다크모드가 적용되지만,
**내 코드에서 Tailwind의 `dark:` 프리픽스를 함께 쓰고 싶다면** 한 줄 설정이 필요합니다:
...
이제 grannyshot-ui 컴포넌트와 Tailwind `dark:` 클래스가 동시에 동작합니다:

```tsx
// grannyshot-ui 컴포넌트 — 자동 다크모드 (설정 불필요)
<Button variant="primary">Submit</Button>

// 내 코드에서 dark: 사용 — @variant 설정 필요
